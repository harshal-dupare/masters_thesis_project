...
This problem can be avoided if we can train the network to also minimize the absolute condition number, $cond_{abs}(\mathcal{N},x)$ over all the input space or minimize the maximum relative condition number over a subset $X_{I} \subseteq X$, $\underset{x \in X_{I}}{max}\hspace{1mm} cond_{rel}(\mathcal{N},x)$. We can also provide the constrains on their maximum values and model accordingly.\newline
