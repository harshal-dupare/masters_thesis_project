...
class MO_NPM_1:
    ...
    def run(self, learning_rate_A, learning_rate_beta, iter_max=100, logger:Logger=None):
        ...
        for iter in tqdm(range(iter_max), desc="MO_NPM_2"):
            ...
            T_i =  0.5*learning_rate_A *(np.matmul(deltaA.T,A_i) - np.matmul(A_i.T,deltaA))
            Q = cayley_transformation(T_i)
            A_i = np.matmul(A_i, Q)
            beta_i = beta_i + learning_rate_beta*deltabeta
            R_i = self.data_handler.compute_reward(beta_i, A_i)
            L_i = self.data_handler.compute_loss(beta_i, A_i)
            if logger is not None:
                logger.log_RL(R_i,L_i)
            if R_i > R_opt:
                A_opt = A_i
                beta_opt = beta_i
                R_opt = R_i
...
class MO_NPM_2:
    ...
    def run(self, learning_rate_A, learning_rate_beta, iter_max=100, logger:Logger=None):
        ...
        for iter in tqdm(range(iter_max), desc="MO_NPM_2"):
            ...
            deltaA = float(1-R_i)*R_grad_A - float(L_i)*L_grad_A - 0.5*O_grad_A
            deltabeta = float(1-R_i)*R_grad_beta - float(L_i)*L_grad_beta
            A_i = A_i + learning_rate_A * deltaA
            beta_i = beta_i + learning_rate_beta*deltabeta
            R_i = self.data_handler.compute_reward(beta_i, A_i)
            L_i = self.data_handler.compute_loss(beta_i, A_i)
            if logger is not None:
